    ...
    for (int i = 0; i < rows; ++i)
    {
        ...
        for (int j = 0; j < cols; ++j)
        {
            if (matrix.find(i) != matrix.end() && matrix[i].find('A' + j) != matrix[i].end()) {
                ...
                if (matrix[i]['A' + j]->getclassname() == "Animal") {
                    // If class name is Animal, check getWeightToHarvest
                    auto animal=matrix[i]['A' + j];
                ...
                } else if (matrix[i]['A' + j]->getclassname() == "Plant") {
                    // If class name is Plant, check getDurationToHarvest
                    auto plant=matrix[i]['A' + j];
...
int main(){
    ...
    Plant* sample = new Plant("ABD", "ABC", "TYPE", 0, 15, 15, "A02");
